...
def _require_mapping(value: Any, step: str, field: str) -> Mapping[str, Any]:
...
def _as_float(value: Any, step: str, field: str) -> float:
...
def file_ingestion(input_payload: MutableMapping[str, Any]) -> StepOutcome:
    ...
    for index, file_payload in enumerate(files):
        ...
        if not isinstance(file_payload["records"], list):
            raise HVOSStepError(
                code="INVALID_INPUT",
                message="file records must be a list",
                step=step,
                details={"file": file_payload["id"], "field": "records"},
            )
...
def data_cleaning(input_payload: MutableMapping[str, Any]) -> StepOutcome:
    ...
    for file_payload in input_payload["ingested_files"]:
        cleaned_records = []
        for record_index, record in enumerate(file_payload.get("records", [])):
            if not isinstance(record, Mapping):
                raise HVOSStepError(
                    code="INVALID_INPUT",
                    message="Each ingested record must be a mapping",
                    step=step,
                    details={"file": file_payload["id"], "record_index": record_index},
                )
...
def data_structuring(input_payload: MutableMapping[str, Any]) -> StepOutcome:
    ...
    subject = _require_mapping(input_payload["subject"], step, "subject")
    _require_keys(subject, step, ["metric"])
    ...
    return StepOutcome(
        status="ok",
        payload={"structured_dataset": contract.structured_dataset},
        assumptions=[],
    )
...
def data_gate(input_payload: MutableMapping[str, Any]) -> StepOutcome:
    ...
    subject = _require_mapping(dataset["subject"], step, "structured_dataset.subject")
    subject_metric = _as_float(subject.get("metric"), step, "subject.metric")
    if subject_metric <= 0:
        return StepOutcome(
            status="fail",
            payload={"gate_status": "FAIL", "reasons": ["Subject metric must be > 0"]},
            assumptions=[],
        )
    ...
    for idx, comp in enumerate(comparables):
        ...
        metric = _as_float(comp["metric"], step, f"comparables[{idx}].metric")
        if metric <= 0:
            reasons.append(f"Comparable {idx} metric must be > 0")
            continue
        price = _as_float(comp["price"], step, f"comparables[{idx}].price")
        if price <= 0:
            reasons.append(f"Comparable {idx} price must be > 0")
            continue
        ratios.append(price / metric)
        valid_indexes.append(idx)

    if reasons:
        return StepOutcome(
            status="fail",
            payload={"gate_status": "FAIL", "reasons": reasons},
            assumptions=[],
        )
...
def adjustment(input_payload: MutableMapping[str, Any]) -> StepOutcome:
    ...
    rules = _require_mapping(rules, step, "adjustment_rules")
    _require_keys(rules, step, ["metric_factor"])
    factor = _as_float(rules["metric_factor"], step, "adjustment_rules.metric_factor")

    adjusted_comps = []
    assumptions: List[str] = []
    filtered_outliers: List[Dict[str, Any]] = []
    for idx, comp in enumerate(comparables):
        ...
        if has_assumption_flag:
            assumptions.append(
                f"Comparable {comp.get('name', idx)} assumption_flag: {assumption_flag}"
            )
    ...
    return StepOutcome(
        status="ok",
        payload={"adjusted_comps": adjusted_comps, "filtered_outliers": filtered_outliers},
...
def _compute_dcf_value(step: str, dcf_inputs: Mapping[str, Any]) -> float:
    ...
    fcf1 = _as_float(dcf_inputs["fcf1"], step, "dcf_inputs.fcf1")
    discount_rate = _as_float(
        dcf_inputs["discount_rate"], step, "dcf_inputs.discount_rate"
    )
    growth_rate = _as_float(dcf_inputs["growth_rate"], step, "dcf_inputs.growth_rate")
    ...
    if fcf1 <= 0:
        raise HVOSStepError(
            code="INVALID_INPUT",
            message="fcf1 must be greater than zero",
            step=step,
            details={"fcf1": fcf1},
        )
...
def valuation(input_payload: MutableMapping[str, Any]) -> StepOutcome:
    ...
    market_values = [
        _as_float(comp["adjusted_price"], step, f"adjusted_comps[{idx}].adjusted_price")
        for idx, comp in enumerate(adjusted_comps)
    ]
    ...
    return StepOutcome(
        status="ok",
        payload={"valuation_results": contract.valuation_results},
        assumptions=assumptions,
    )
...
def reconciliation(input_payload: MutableMapping[str, Any]) -> StepOutcome:
    ...
    weighted_components = []
    market_value = (
        results.get("market", {}).get("value")
        if isinstance(results.get("market"), Mapping)
        else None
    )
    dcf_value = (
        results.get("dcf", {}).get("value")
        if isinstance(results.get("dcf"), Mapping)
        else None
    )

    if market_value is not None:
        weighted_components.append((_as_float(market_value, step, "market.value"), 0.7))
    if dcf_value is not None:
        weighted_components.append((_as_float(dcf_value, step, "dcf.value"), 0.3))
